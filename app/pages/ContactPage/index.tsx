...
const ContactPage = () => {
  const sectionInfo = [
    ...
    { id: 'contact-location', title: 'Or come visit us', content: '123, The Solar Flare, Oasis Spring', icon: 'location_on' },
  ];
      ...
      <h2>Interested in buying a lot?</h2>
        {sectionInfo.map((item)=>
          <section className="mt-5" key={item.title}>
            <h3>
              <span className="material-symbols-rounded me-2" aria-hidden="false">{item.icon}</span>
              {item.title}
            </h3>
